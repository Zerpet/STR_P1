...
/* unsigned char gCarreta[4][4] = { {195,179,192, 32},
								 {196, 32,196,'O'},
								 {180,179,217, 32},
								 { 32,'<', 32, 32}};
								 
unsigned char gDepositoLleno[4][3] = { {195,179,192},
									   {196, 32,196},
									   {196, 32,203},
									   {180,179,217}};                               

unsigned char gDepositoVacio[4][3] = { {179,195,192},
									   { 32,196,196},
									   { 32,196,203},
									   {179,180,217}};                               

unsigned char gFilas[22][15] ={{ 32, 32, 32, 32, 32, 32, 32, 32, 32, 32,219,219,219,219,219},
...
int crearInferior(struct control_data_backup_t data, char inferior[PANTALLA_SIZE])
{
    ...
    sprintf(inferior,
       "\n"
       "\t\t\t\tTime:%s sec\n"
       "\t  Speed:%s\tFloor:%s\tTime Mix:%s\tIsDark:%s \n"
       "\t  Gas:%d\t\tBrake:%d\t\tMixer:%d  \tLights:%s \n",
       strTime,
       strSpeed,rute,strMixer,isDark,
       data.gas,data.brake,data.mixer,light);
    return (0);
}
...
int displaySlope(int slope) 
{
    ...
    timeWait.tv_sec = TIME_DISPLAY_SEC + retraso();
